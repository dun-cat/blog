...
3. **对网络环境的要求**。首先 React Native 集成到 Native 端是基于源码编译，而源码的依赖库很多是需要翻墙下载的。比如：React Native 0.68 版本及之后，需要用到：`folly`、`glog`、`boost`等需要网络下载包；
...
[Hermes](https://hermesengine.dev/)是 Facebook 专门针对 React Native 做过优化的 Javascript 引擎，在 React Native .70 版本中 Hermes 作为**捆绑的默认内置引擎**。

在这之前 React Native 一直使用 [JavaScriptCore（JSC）](https://webkit.org/) 作为其执行引擎，考虑到 iOS 平台内置 JSC Android 平台表现一致，官方维护了一个构建 Android 平台 JSC 的[脚本仓库](#react-native-communityjsc-android-buildscripts)。当然如果你的 React Native 版本 >= 0.60.4，可以把 Hermes 作为 React Native 的 Javascript 引擎使用。
...
有人担心 Hermes 内置到 iOS App 内，是否可以过审，至少目前社区内尚无反馈。从[这里](https://github.com/facebook/hermes/issues/34)有相关的讨论，可以知道 Apple 并未强制要求 App 使用 JSC，Apple 更关注的是代码下发后的动态执行。
