## Git分支管理之清除 
### 简介
对于分布式的git，远程分支多了，不利于接管的开发人员立即投入开发。而在公司自动化不是很完善的时候，手动处理分支变得家常。
...
会分为三步：
1. `git branch --merged`：列出已经合并到当前分支的分支列表，确保删除安全；
2. `egrep -v "(^\*|master|dev)"`：排除不想删除的关键分；
3. `xargs git branch -d`：执行删除操作；
