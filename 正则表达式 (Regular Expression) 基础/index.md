...
除了元字符，POSIX 还提供了`字符类` (character classes) 来简化正则的编写。字符类也被叫做字符集或字符组，源于字符类是一组包含在`方括号`中的字符。
...
PCRE 是由 C 实现的一个正则表达式引擎库，PCRE 的语法比任何一种 POSIX 正则表达式风格 (BRE、ERE) 和许多其他正则表达式库的语法都要强大和灵活得多。
