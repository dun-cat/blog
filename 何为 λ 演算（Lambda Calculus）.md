...
在 λ 演算中没有值的概念，对于数学`值（自然数）`的表示，λ 演算也有对应的[邱奇数](/articles/lambda-calculus/#数字表达式)表示。邱奇数也是一个函数。在λ 演算中`一切皆函数`的理念深入任何地方。

### 数的表示

在数学中，`3`表示一个抽象符号，举个简单例子：

`🍎 🍎 🍎`，这里的苹果个数，抽象化的数学符号表示就是：`3`。同样`🍌 🍌 🍌`，这里的香蕉个数，抽象化的数学符号也是：`3`。所以我们认为前面的`3`等价（“=”表示）于后面的`3`，即 $3=3$。而`3`这个符号我们通过其它符号基于数学各种运算法则也能得出，比如：`1+2=3`、`4-1=3`。
...
邱奇数对于自然数 $0$ 的初始定义为 $λy.λx.x$，后继函数表示为：$S ≡ λn.λy.λx.y\ (n\ y\ x)$
...
通过 λ 演算法则的[α-变换](/articles/lambda-calculus/#α-变换)和[β-归约](/articles/lambda-calculus/#β-归约)来理解如何通过自然数`0`来构造自然数`1`的：

也许你对上面的表达式感到迷惑。ok，我们通过 Javascript 语法来看待上面的表达式，首先需要知道的是创建自然数有两个先决关键条件：`定义 0 是自然数`和`后继函数`。形式化表示就是：$x = 0$ 和 $f(x) = x + 1$；

下面是邱奇数对`0`、`1`、`2`、`3`的自然数的定义转为 Javascript 语法的书写方式。
